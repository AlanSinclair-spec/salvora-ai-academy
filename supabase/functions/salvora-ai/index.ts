...
function validateMessages(messages: unknown): AIMessage[] {
...
serve(async (req) => {
  ...
  try {
    ...
    if (!LOVABLE_API_KEY) {
      ...
      return new Response(
        JSON.stringify({ error: "El servicio de IA no está disponible", code: "SERVICE_CONFIG_ERROR" }),
    ...
    console.log(`Salvora AI request: action=${action}`);

    let aiMessages: AIMessage[] = [];

    switch (action) {
      case "chat":
        aiMessages = validateMessages(messages);
...
Usa estos nombres de niveles: Excelente, Bueno, En desarrollo, Necesita mejora`
    ...
    if (!response.ok) {
      ...
      if (response.status === 402) {
        return new Response(
          JSON.stringify({ error: "Créditos de IA agotados. Contacta al administrador.", code: "PAYMENT_REQUIRED" }),
